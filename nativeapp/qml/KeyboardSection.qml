...
Card {
    ...
    Column {
        ...
        Column {
            ...
            Text {
                width: parent.width
                text: "Changing layout restarts the interface to take effect."
                font.pixelSize: 20
                color: "#222"
            ...
            Row {
                ...
                ActionButton {
                    text: kbCard.kbConnected ? "Reconnect" : "Reconnect"
                    enabled: !kbCard.busy && kbCard.kbMac !== ""
                    onClicked: {
                        kbCard.busy = true
                        kbCard.statusText = "Reconnecting — wake the keyboard if needed..."
                        sendRequest("reconnect_keyboard", {"mac": kbCard.kbMac}, function(resp) {
                            kbCard.busy = false
                            if (resp.ok) {
                                kbCard.kbConnected = !!resp.data.connected
                                kbCard.statusText = kbCard.kbConnected
                                    ? "Connected" : "Still disconnected"
                            } else {
                                kbCard.statusText = "Error: " + (resp.error || "Unknown")
                            }
                        })
