...
python cli.py repair-ui                   # same as enable-settings-ui (easy recovery name)
```
...
**Settings → Help** (after `enable-settings-ui` / `repair-ui`): status, Bluetooth, keyboard reconnect, language layout (US / US Intl / UK / DE / FR / IT / ES), pointer knobs. Pairing stays on the host CLI (`scan` / `pair`).
...
`python cli.py status` reports a `=== settings_ui ===` block. If it says `needs_repair`, run `repair-ui`.
...
3. Run **`python cli.py repair-ui`** (or the older alias `python cli.py pointer enable-settings-ui`).
